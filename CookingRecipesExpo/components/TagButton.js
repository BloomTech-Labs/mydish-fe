...
export default function TagButton(props) {
    const {tag, color, setColor, toggleBackgroundColor, tagsIncluded, recipe, setRecipe} = props;

    return (
        <TouchableOpacity style={color.active.includes(tag) ? styles.tagButtonPressed : styles.tagButtons} 
            onPress = {() => {toggleBackgroundColor(tag,color,setColor); tagsIncluded(tag,recipe,setRecipe)}}>
