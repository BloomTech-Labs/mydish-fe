...
const CookBookFolder = (props) =>{
    ...
    const grab =  async () =>{
        ...
        try {
            const courses = await axiosAuth.get(`https://recipeshare-development.herokuapp.com/cookbook?category=${course}`)
            // console.log('res.data recipes by course', courses.data);
            setFolder(courses.data);
            const {data} = await axiosAuth.get(`https://recipeshare-development.herokuapp.com/recipes/all`);
            const childrenRecipes = data.filter(rec => rec.ancestor);
            console.log('children with an ancestor in <CookBookFolder>', childrenRecipes);
...
//       .catch(err => console.log(err));
    ...
    return(
        <View style={{alignItems : 'center', justifyContent: 'center'}}>
            {folder.length ? 
            <RecipeList recipes={folder} forks={children} courseType={course}/> :
            <>
